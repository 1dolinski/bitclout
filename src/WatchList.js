import { React, Fragment, useState, useEffect, useCallback } from "react";
import { get, useForm } from "react-hook-form";
import { DateTime } from "luxon";
import "firebase/compat/firestore";

import Header from "./Header.js";

import api from "./api.js";

const hasPremiumAccess = () =>
  localStorage.getItem("bitcloutoffersPremiumCalendar") === "DiamondCalendar";

const NEVER_POSTED = 0;

function WatchListCreator(props) {
  const { creator } = props;
  return (
    <div
      key={creator.creator}
      class="mt-4 mb-4 flex items-center justify-center"
    >
      <div
        class="w-20 h-20 max-w-xs rounded mr-2"
        style={{
          "background-image": `url("${creator.data.creator.Profile.ProfilePic}")`,
          "background-size": "cover",
        }}
      ></div>
      <a
        href={`https://www.bitclout.com/u/${creator.creator}`}
        class="underline flex-grow font-semibold text-blue-500"
      >
        ${creator.creator}
      </a>
      <div>
        {creator.lastPosted === NEVER_POSTED
          ? "-"
          : DateTime.fromSeconds(creator.lastPosted / 1000000000).toRelative()}
      </div>
    </div>
  );
}

export default function WatchList() {
  const bcoKey = "bitcloutoffers-watchlist";

  const [watchlist, setWatchlist] = useState([]);
  const [refreshing, setRefreshing] = useState(false);

  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm();

  useEffect(() => {
    const script = document.createElement("script");
    script.setAttribute("embedclout", "bitcloutoffers");
    script.setAttribute("template", "light");
    script.setAttribute("position", "default");
    script.setAttribute("size", "detailed");
    script.src = "https://www.embedclout.com/badge/badge.js";

    document.querySelector("#embed").appendChild(script);
  }, []);

  const defaultWatchList = {
    creators: {},
  };

  const getWatchlist = () => JSON.parse(localStorage.getItem(bcoKey));

  const ensureDefault = () => {
    const list = getWatchlist() || defaultWatchList;
    setWatchlist(list);
    localStorage.setItem(bcoKey, JSON.stringify(list));
  };
  const creatorList = () => {
    return getWatchlist() ? Object.keys(getWatchlist().creators) : []
  }


  const clearWatchList = () => {
    console.log("clearing watchlist");
    localStorage.setItem(bcoKey, JSON.stringify(defaultWatchList));
    setWatchlist(defaultWatchList);
  };

  const setCreator = (username, value) => {
    let list = getWatchlist();
    list["creators"][username] = value;
    console.log("set creator", value);
    localStorage.setItem(bcoKey, JSON.stringify(list));
  };

  const addCreator = async (username) => {
    setRefreshing(true);
    let simpleUsername = username.replace(/@|\$/g, "");
    let creatorObj = await fetchCreator(simpleUsername);

    console.log("adding creator?", creatorObj);
    if (creatorObj === undefined) {
      setRefreshing(false);
      return;
    }

    let list = getWatchlist();
    console.log("add creator", creatorObj);
    list["creators"][simpleUsername] = creatorObj;

    localStorage.setItem(bcoKey, JSON.stringify(list));
    refreshCreators();
  };

  const removeCreator = (username) => {
    let watchList = getWatchlist();
    delete watchList["creators"][username];
    localStorage.setItem(bcoKey, JSON.stringify(watchList));
    setWatchlist(watchList);
  };

  const fetchCreator = async (username) => {
    // no creator
    let creator = await api.getSingleProfile(username);
    if (creator.Profile === undefined) {
      alert("Sorry, we couldn't find that user!");
      return;
    }

    // creator with no posts
    let creatorPosts = await api.getPostsForPublicKey(username);

    if (creatorPosts.Posts === null) {
      return {
        creator: username,
        lastPosted: NEVER_POSTED,
        data: {
          creator: creator,
          lastPost: null,
        },
      };
    }

    // creator has posts
    let lastCreatorPost = await api.getPost(creatorPosts["LastPostHashHex"]);

    return {
      creator: username,
      lastPosted: lastCreatorPost.PostFound.TimestampNanos,
      data: {
        creator: creator,
        lastPost: lastCreatorPost,
      },
    };
  };

  const refreshCreators = async () => {
    // clearWatchList();
    ensureDefault();
    setRefreshing(true);

    console.log("refreshCreators creatorList", creatorList());

    const promises = creatorList().map((creator) => fetchCreator(creator));
    const creators = await Promise.all(promises)

    setRefreshing(false);
    const list = {};
    creators.forEach(creator => list[creator.creator] = creator);
    setWatchlist({ creators: list });
  };

  useEffect(() => {
    refreshCreators();
  }, []);

  return (
    <Fragment>
      <Header
        title="Build Your BitClout"
        subtitle="Watch List"
        description="Check in on your favorite creators. When were they last active?"
        unsplashId="photo-1500462918059-b1a0cb512f1d"
      />

      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8 lg:items-center lg:justify-between">
        {/* <div>
          <PremiumCode />
        </div> */}

        <div class="text-3xl font-bold">See When Creators Were Last Active</div>
        {
          <div class="h-12 font-bold text-gray-500">
            {refreshing && "💎 Refreshing... "}
          </div>
        }
        {
          <form
            class="grid grid-cols-12 gap-2"
            onSubmit={handleSubmit(async (data) => {
              await addCreator(data.username);
            })}
          >
            <input
              placeholder="Type a username..."
              class="col-span-6 border inline-flex items-center justify-center px-5 py-3 border border-indigo text-base font-medium rounded-md text-indigo"
              type="input"
              {...register("username", { required: true })}
            />
            <input
              type="submit"
              value="Add User"
              class="col-span-3 inline-flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
            />
          </form>
        }
        <div>
          {watchlist &&
            watchlist.creators &&
            Object.values(watchlist.creators)
              .sort((a, b) => {
                console.log(
                  "sorting?",
                  a.lastPosted,
                  b.lastPosted,
                  a.creator,
                  b.creator
                );
                return a.lastPosted - b.lastPosted;
              })
              .map((creator) => creator.creator)
              .map(
                (creator) =>
                  creator && (
                    <div class="flex">
                      <div class="flex-grow">
                        <WatchListCreator
                          creator={watchlist["creators"][creator]}
                        />
                      </div>
                      <button
                        class="underline ml-4"
                        onClick={() => removeCreator(creator)}
                      >
                        Remove
                      </button>
                    </div>
                  )
              )}

          {watchlist && creatorList().length == 0 && (
            <div class="text-lg mt-16 font-semibold">Add your first user! 💎 </div>
          )}
        </div>
        <button
          class={`col-span-3 mt-24 bg-gray-300 hover:bg-gray-400 text-white font-bold py-3 px-3 rounded`}
          onClick={() => clearWatchList()}
        >
          Clear Watchlist
        </button>

        {/* <div id="embed"></div> */}
      </div>

      <div className="bg-gray-50">
        <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8 lg:flex lg:items-center lg:justify-between">
          <h2 className="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
            <span className="block">Thanks To BitClout's</span>
            <span className="block text-indigo-600">OG Hype Creators</span>
          </h2>
          <div className="mt-8 flex lg:mt-0 lg:flex-shrink-0">
            <div className="inline-flex rounded-md shadow mr-4">
              <a
                href="https://www.bitclout.com/u/brockpierson"
                target="_blank"
                className="inline-flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
              >
                <div
                  class="h-20 w-20 max-w-xs max-h-xs rounded mr-2"
                  style={{
                    backgroundImage: `url("data:image/webp;base64,")`,
                    backgroundSize: "cover",
                  }}
                ></div>
                Brock Pierson
              </a>
            </div>
            <div className="inline-flex rounded-md shadow">
              <a
                href="https://www.bitclout.com/u/RajLahoti"
                target="_blank"
                className="font-semibold inline-flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
              >
                <div
                  class="h-20 w-20 max-w-xs max-h-xs rounded mr-2"
                  style={{
                    backgroundImage: `url("data:image/webp;base64,")`,
                    backgroundSize: "cover",
                  }}
                ></div>
                Raj Lahtoi
              </a>
            </div>
          </div>
        </div>
      </div>
      <div class="mt-16 mb-16 pull-right">
        <span id="embed"></span>
      </div>
    </Fragment>
  );
}

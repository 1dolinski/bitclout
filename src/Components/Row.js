...
const dateLabel = (record) => {
    ...
    if (record.get('DateTime')) {
        return DateTime.fromISO(record.get('DateTime')).toFormat('MMM dd H:mm ZZZZ')
    }
}
...
export default function Row(props) {
    ...
    <div className="flex">
        <div class="flex-1">
            ...
            {props.record.get('Calendar') && 
                <span className="ml-2 text-gray-400">{DateTime.fromISO(props.record.get('Calendar')).toRelative()}</span>
            }
        ...
        <div className="py-4 pl-8 pr-16 font-semibold">
            {props.record.get('Highlight?') ? 'Featured' : monthDay(props.record.get('Created At'))}
        </div>

        <div className="py-4 pl-8 pr-8 w-24"> 
